...
class PNDMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        cur_sample: Optional[torch.Tensor] = None,
    ...
    def step_plms(
        ...
        if not self.config.skip_prk_steps and len(ets) < 3:
            raise ValueError(
        ...
        prev_timestep = timestep - self.config.num_train_timesteps // num_inference_steps
        ...
        if counter != 1:
            ...
            timestep = timestep + self.config.num_train_timesteps // num_inference_steps

        if len(ets) == 1 and counter == 0:
            model_output = model_output
            cur_sample = sample
        elif len(ets) == 1 and counter == 1:
            model_output = (model_output + ets[-1]) / 2
            sample = cur_sample
            cur_sample = None
        elif len(ets) == 2:
        ...
        if not return_dict:
            return (prev_sample, ets, cur_sample)

        return SchedulerOutput(prev_sample=prev_sample, ets=ets, cur_sample=cur_sample)
