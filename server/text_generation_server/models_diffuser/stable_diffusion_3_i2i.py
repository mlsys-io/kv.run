...
class Stable_Diffusion_3_i2i_Model:
    ...
    def prefill(self, batch: StableDiffusion3Batch):
        ...
        image = self.model.image_processor.preprocess(batch.image)
        
        timesteps, num_inference_steps = retrieve_timesteps(
            self.model.scheduler, batch.num_inference_steps, self.device, batch.timesteps,
        )
        timesteps, num_inference_steps = self.model.get_timesteps(num_inference_steps, batch.strength, self.device)
...
if __name__ == "__main__":
    ...
    req = Stbale_Diffusion_Request(0, "A dog", Image.open("test.png"))
    req2 = Stbale_Diffusion_Request(1, "A cat", Image.open("test.png"))
